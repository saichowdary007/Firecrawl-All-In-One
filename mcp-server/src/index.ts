...
class FirecrawlMCPServer {
  ...
  private creditWarningThreshold = 0.8; // 80% of limit
  private retryConfig = {
    maxRetries: 3,
  ...
  private sseClients = new Map<string, any>(); // SSE clients for real-time updates
  ...
  private addSSEClient(clientId: string, client: any) {
  ...
  private broadcastSSEUpdate(operationId: string, data: any) {
    const update = {
      operationId,
  ...
  private updatePerformanceMetrics(operation: string, success: boolean, duration: number) {
  ...
  private logPerformanceSummary() {
    ...
    this.log(LogLevel.INFO,
      `Performance Summary: ${metrics.totalRequests} total, ${successRate}% success, ${metrics.averageResponseTime.toFixed(0)}ms avg response`,
      'performance_monitoring'
    );
  }

  // Enhanced logging system
  private log(level: LogLevel, message: string, operation?: string, duration?: number, creditUsage?: number) {
  ...
  private async handleCheckCrawlStatus(args: any) {
    const response = await this.executeWithRetry(
      () => this.client.get(`/v1/crawl/${args.id}`),
      "check_crawl_status"
    );
  ...
  private async handleDeepResearch(args: any) {
    const response = await this.executeWithRetry(
      () => this.client.post("/v2/deep-research", {
        query: args.query,
  ...
  private async handleGenerateLLMsTxt(args: any) {
    const response = await this.executeWithRetry(
      () => this.client.post("/v2/generate-llmstxt", {
        url: args.url,
  ...
  private async handleBatchScrape(args: any) {
    const response = await this.executeWithRetry(
      () => this.client.post("/v2/batch/scrape", {
        urls: args.urls,
        formats: args.formats || ["markdown"],
        onlyMainContent: args.onlyMainContent,
        includeTags: args.includeTags,
  ...
  private async handleCheckBatchStatus(args: any) {
    const response = await this.executeWithRetry(
      () => this.client.get(`/v2/batch/scrape/${args.id}`),
      "check_batch_status"
    );
  ...
  private async handleSubscribeUpdates(args: any) {
